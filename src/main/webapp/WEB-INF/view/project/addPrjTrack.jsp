...
<head lang="en">
    ...
    <script type="text/javascript">
     function prjTrack(tracktime,topic,trackimg,projectId){
	...
	function getNowFormatDate() {
     ...
     function submit()     {
		...
		//var timestamp2=Date.parse(new Date(stringTime));
    	 var data=JSON.stringify(new prjTrack(timestamp2,$("#devise").val(),$('input[name="prjPic"]').val(),"${projectId}"));
    	 $.ajax( {
 	        type : "post",  
 	        url : "saveprjTrack",  
 	        ...
 	        success : function(data) {  
 	            ...
 	            location.href="${contextPath}/ground/prjTrack?projectId=${projectId}";
 	        },
 	        error:function(data){
    ...
    <script type="text/javascript">
    	...
    	//文件普通上传（非压缩。在客户端不支持压缩或者图片小于100k，不需要压缩时调用）
    	var uploadFile=function(fileInputId){
    		 var fileField = $('#'+fileInputId)[0];
    		 console.log(fileField.parentInput);
             var parentName = fileField.parentInput.attr('name');
             var upResult = ProjectCreate.fileParentChildren[parentName];
             var fileFieldName = fileField.parentInput.attr('title');
             var url="";
             upResult.status='uploading';
             //console.log($('#'+fileInputId+'FileForm'));
             //$('#'+fileInputId+'Uping').text('--正在上传...');
             console.log(fileInputId);
             url="${contextPath}/ground/uploadMultiFilesForPrjTrack"		 
             uploading(fileInputId);
    		 $('#'+fileInputId+'FileForm').ajaxSubmit({
                 url:url,
                 dataType:'json',
                 success: function(data, status){     
                     upResult.status='uploaded';
                     if(data.exceptionMsg||!data.result){
                    	 //$('#'+fileInputId+'Uping').text('--上传失败');
                    	 uploadFailed(fileInputId);
                    	 fileField.isUploading = false;
                     }else{
                    	 var fileNames = data.result; 
	                     upResult.name = fileNames;
	                     //var fileNameStr = '';
	                     ...
	                     if(fileNames&&fileNames.length<2){
		                     ...
		                     fileField.parentInput.val(fileNames);
	                     }
    	                 //$('#'+fileInputId+'Uping').text('--上传成功');
    	                 uploadSuccess(fileInputId,fileNames);
    	                 fileField.isUploading = false;
                     }
                 },
                 error: function (data, status, e){
                	 ...
                	 uploadFailed(fileInputId);
                	 upResult.status='uploaded';
                     Box.autoClose(fileFieldName+'上传失败，请检查网络');
                	 fileField.isUploading = false;
                 }
             });
    	};
    	//文件压缩上传
    	var uploadCompressFile=function(fileInputId,imgData){
    		 var fileField = $('#'+fileInputId)[0];
    		 console.log(fileField.parentInput);
             var parentName = fileField.parentInput.attr('name');
             var upResult = ProjectCreate.fileParentChildren[parentName];
             ...
             //$('#'+fileInputId+'Uping').text('--正在上传...');
            // console.log(fileInputId);
             var url="${contextPath}/ground/uploadComrpessFileForPrjTrack";
             uploading(fileInputId);
    		 $.ajax({
                 url:url,
                 type:'post',
    	...
    	//上传进度条及其相关处理
    	var bindingUpings = {};
    	function bindingUping(fileInputId){
    		//console.log(fileInputId);
			var fileInput = bindingUpings[fileInputId];
			var $upbar = $(fileInput).find('div.upbar');
			var $upmsg = $(fileInput).children('.upMsg'); 
			var width = $upbar.width();
			var intev = 1000;
			//进度条速度调整
			if(width<30){
				width+=2;
				...
				//94%后停止进度条，等待上传结果（失败或成功）
				return ;
			}
			$upmsg.text(width+'%');
			$upbar.width(width);
			//console.log(width);
			setTimeout("bindingUping('"+fileInputId+"')",intev);
    	}
    	//文件开始上传时调用
    	function uploading(fileInputId){
    		var $uping = $("#"+fileInputId+"Uping");
    		$uping.find('div.upbar').width(0);
    		$uping.children('.upbarContainer').show();
    		bindingUping(fileInputId);
    	}
    	//文件上传成功后调用
    	function uploadSuccess(fileInputId,fileName){
    		var fileInput = bindingUpings[fileInputId];
			var $upbar = $(fileInput).find('div.upbar');
			var $upmsg = $(fileInput).children('.upMsg'); 
			$upbar.width('100%');
			$upmsg.text('上传成功');
			var fileList = upFileList[fileInputId];
			if(!fileList){
				return;
			}
			var fileuping = fileList['uping'];
			if(fileuping && ''===fileList[fileuping]){
				fileList[fileuping] = fileName;
				delete fileList['uping'];
				var fileNames = '';
				for(var i in fileList){
					if(fileList[i]=='uping'||i=='uping'){
						continue;
					}
					fileNames+=',';
				...
				$('input[name="'+fileInputId+'"]').val(fileNames);
			}
    	}
    	//文件上传失败后调用
    	function uploadFailed(fileInputId){
    		var fileInput = bindingUpings[fileInputId];
			var $upbar = $(fileInput).find('div.upbar');
			var $upmsg = $(fileInput).children('.upMsg');
			//重置file input，便于失败后激活上传
			$('#'+fileInputId).val("");
			$upbar.width(100);
			var fileList = upFileList[fileInputId];
			if(fileList){
				var upingNum = fileList['uping'];
				//console.log(upingNum);
				if(upingNum && !isNaN(upingNum)){
					//console.log('deleted img');
					changeImg(fileInputId,upingNum);
				}
			}
			$(fileInput).children('.upbarContainer').hide(100);
			$upmsg.text('上传失败');
    	}
	...
	*/
	...
	function deleteImg(obj){
		...
		$.each($("#proImg").children(),function(i,data){
			name = $(data).attr("src")
			names = name.substring(name.lastIndexOf("prjTrk/"))+","+names;
...
function downloadImage(type){
	$.each(images.serverId,function(i,data){
		var media_id = data;
		$.ajax({
			url:"${contextPath}/ground/doUploadPrjTrk",
			type:"post",
			dataType:"json",
			...
			success:function(data){
				...
				if(type=="prjPic"){
					$("#proImg").append('<img src="${contextPath}/pics/'+ fileName +'" onclick="deleteImg(this);"/>');
