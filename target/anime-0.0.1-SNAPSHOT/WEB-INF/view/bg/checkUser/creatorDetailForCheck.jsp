...
<div class="pd-20">
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				<tr class="text-c">
					...
					<c:if test="${userFullInfo.userName==null||userFullInfo.userName==''}">
							  <td></td>
						   </c:if>
				   	<c:if test="${userFullInfo.userName!=null&&userFullInfo.userName!=''}">
					...
					<c:if test="${userFullInfo.gender==2}">
				     ...
				     <c:if test="${userFullInfo.gender==null||userFullInfo.gender==''}">
				    	<td></td>
				    </c:if>
				    <c:if test="${userFullInfo.certification==null||userFullInfo.certification==''}">
							  <td></td>
						   </c:if>
				   	<c:if test="${userFullInfo.certification!=null&&userFullInfo.certification!=''}">
							  <td>${userFullInfo.certification}</td>
						   </c:if>	
					 <c:if test="${userFullInfo.nickName==null||userFullInfo.nickName==''}">
							  <td></td>
						   </c:if>
				   	<c:if test="${userFullInfo.nickName!=null&&userFullInfo.nickName!=''}">
					...
					<c:if test="${userFullInfo.companyName==null||userFullInfo.companyName==''}">
							  <td></td>
						   </c:if>
				   	<c:if test="${userFullInfo.companyName!=null&&userFullInfo.companyName!=''}">
				    <td>${userFullInfo.companyName}</td>
						   </c:if>
					<c:if test="${userFullInfo.position==null||userFullInfo.position==''}">
							  <td></td>
						   </c:if>
				   	<c:if test="${userFullInfo.position!=null&&userFullInfo.position!=''}">
				    <td>${userFullInfo.position}</td>
						   </c:if>
					<c:if test="${userFullInfo.telphoneNumber==null||userFullInfo.telphoneNumber==''}">
							  <td></td>
						   </c:if>
				   	<c:if test="${userFullInfo.telphoneNumber!=null&&userFullInfo.telphoneNumber!=''}">
				    <td>${userFullInfo.telphoneNumber}</td>
						   </c:if>
					<c:if test="${userFullInfo.emailAddress==null||userFullInfo.emailAddress==''}">
							  <td></td>
						   </c:if>
				   	<c:if test="${userFullInfo.emailAddress!=null&&userFullInfo.emailAddress!=''}">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				<tr class="text-c">
					<td>${userFullInfo.projname}</td>
					<td>${userFullInfo.projintro}</td>
					<td><img src="${userFullInfo.projphoto}" style="width:80px;height:80px"/></td>
				</tr>
			</tbody>
		</table>
	</div>
			...
			<!-- <label class="form-label col-2"><span class="c-red">*</span>请输入关键字：</label> -->
				<div class="formControls col-10" id="head-input">
					<input class="input-text" style="width:250px;" id="head-rank" type="text" onblur="lostfocus('head-rank');"
						placeholder="输入创业者姓名,昵称，电话等"> <span id="head-bottom" class="bottom-company">确认</span>
				</div>
		</div>
	<!-- 单选按钮 -->
	...
	<div class="identity_add_footer">
		<a class="identity_add_btn" onClick="approve('${userFullInfo.id}')" href="javascript:;" title="通过">通过</a>
...
<script type="text/javascript" src="${contextPath}/bg/lib/ueditor/1.4.3/ueditor.config.js"></script> 
<script type="text/javascript" src="${contextPath}/bg/lib/ueditor/1.4.3/ueditor.all.min.js"> </script> 
<script type="text/javascript" src="${contextPath}/bg/lib/ueditor/1.4.3/lang/zh-cn/zh-cn.js"></script> 
...
function approve(userid){
	...
	/* if(flag==1){
		window.location.href="approve?userId="+userid+"&keySearch="+keySearch;
	}else{
